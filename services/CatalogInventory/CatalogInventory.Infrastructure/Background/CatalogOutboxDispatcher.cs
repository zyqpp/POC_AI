...
namespace CatalogInventory.Infrastructure.Background;
    ...
    ILogger<CatalogOutboxDispatcher> logger) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                if (pendingMessages.Count > 0)
                {
                    ...
                    foreach (var message in pendingMessages)
                    {
                        try
                        {
                            var routingKey = $"catalog.{message.EventType.ToLowerInvariant()}";
                        ...
                        catch (Exception ex)
                        {
                            ...
                            {
                                logger.LogError(ex,
                                    "Catalog outbox message {MessageId} moved to failed after {RetryCount} attempts",
                                    message.MessageId,
                                    message.RetryCount);
                            ...
                            else
                            {
                                logger.LogWarning(ex,
                                    "Catalog outbox publish retry {RetryCount}/{MaxRetryAttempts} for message {MessageId}",
                                    message.RetryCount,
                                    MaxRetryAttempts,
                                    message.MessageId);
            ...
            {
                logger.LogError(ex, "Unexpected catalog outbox dispatcher error.");
            }
