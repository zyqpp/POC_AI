...
namespace PaymentInvoice.Infrastructure.Background;

internal sealed class PaymentOutboxDispatcher(
    IServiceScopeFactory scopeFactory,
    IConfiguration configuration,
    ILogger<PaymentOutboxDispatcher> logger) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
                ...
                if (pendingMessages.Count > 0)
                {
                    ...
                    foreach (var message in pendingMessages)
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            {
                                logger.LogError(ex,
                                    "Payment outbox message {MessageId} moved to failed after {RetryCount} attempts",
                                    message.MessageId,
                                    message.RetryCount);
                            ...
                            else
                            {
                                logger.LogWarning(ex,
                                    "Payment outbox publish retry {RetryCount}/{MaxRetryAttempts} for message {MessageId}",
                                    message.RetryCount,
                                    MaxRetryAttempts,
                                    message.MessageId);
            ...
            {
                logger.LogError(ex, "Unexpected payment outbox dispatcher error.");
            }
