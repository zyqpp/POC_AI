# ERR-007-0001 This product is currently unavailable

Status: `szkielet`; wymagane ręczne uzupełnienie po analizie UI, API i obsługi błędów.

## Identyfikacja

| Atrybut | Wartość |
|---|---|
| ID błędu | `ERR-007-0001` |
| Ekran | [E-007](../E-007__README.md) |
| Nazwa wykryta | `This product is currently unavailable` |
| Typ detekcji | `toast.warning` |
| Źródło | `supply-chain-frontend/src/app/features/catalog/product-list/product-list.component.ts` |
| Status faktu | `do uzupełnienia` |

## Opis Błędu

Do uzupełnienia.

## Warunki Wystąpienia

| Warstwa | Warunek | Status |
|---|---|---|
| UI | do uzupełnienia | `do uzupełnienia` |
| Walidacja frontend | do uzupełnienia | `do uzupełnienia` |
| API/backend | do uzupełnienia | `do uzupełnienia` |
| Baza/integracja | do uzupełnienia | `do uzupełnienia` |

## Testy

- [Macierz testów ekranu](../TC-007_TESTY/TC-007__INDEX.md)
- Dane wywołujące błąd: do uzupełnienia.
- Oczekiwany komunikat: do uzupełnienia.

## Linki

- [Indeks błędów](ERR-007__INDEX.md)
- [Pola ekranu](../P-007_POLA/P-007__INDEX.md)
- [Akcje ekranu](../A-007_AKCJE/A-007__INDEX.md)
