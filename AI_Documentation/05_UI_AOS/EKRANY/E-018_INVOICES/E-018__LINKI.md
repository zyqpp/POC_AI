# E-018 Linki Śladu

Status: `szkielet`; dokument służy jako centralna nawigacja dla ekranu.

## Ekran

- [Opis ekranu](E-018__README.md)
- [Pola UI](P-018_POLA/P-018__INDEX.md)
- [Akcje UI](A-018_AKCJE/A-018__INDEX.md)
- [Błędy i komunikaty](ERR-018_BLEDY/ERR-018__INDEX.md)
- [Dane testowe](TD-018_DANE_TESTOWE/TD-018__INDEX.md)
- [Testy](TC-018_TESTY/TC-018__INDEX.md)

## Powiązania Do Uzupełnienia

| Obszar | Link | Status |
|---|---|---|
| API | [API_INVOICES.md](../../../04_API/API_INVOICES.md) | `do uzupełnienia` |
| Proces | do uzupełnienia po analizie `06_PROCESY` | `do uzupełnienia` |
| Role | [ROLE_INVOICES.md](../../../07_ROLE_I_UPRAWNIENIA/ROLE_INVOICES.md) | `do uzupełnienia` |
| Model danych | do uzupełnienia po analizie `03_MODEL_DANYCH` | `do uzupełnienia` |
| Testy automatyczne | do uzupełnienia po analizie `08_TESTY` | `do uzupełnienia` |
