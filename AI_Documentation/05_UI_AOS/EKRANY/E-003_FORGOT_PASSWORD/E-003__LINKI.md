# E-003 Linki Śladu

Status: `szkielet`; dokument służy jako centralna nawigacja dla ekranu.

## Ekran

- [Opis ekranu](E-003__README.md)
- [Pola UI](P-003_POLA/P-003__INDEX.md)
- [Akcje UI](A-003_AKCJE/A-003__INDEX.md)
- [Błędy i komunikaty](ERR-003_BLEDY/ERR-003__INDEX.md)
- [Dane testowe](TD-003_DANE_TESTOWE/TD-003__INDEX.md)
- [Testy](TC-003_TESTY/TC-003__INDEX.md)

## Powiązania Do Uzupełnienia

| Obszar | Link | Status |
|---|---|---|
| API | [API_IDENTITY.md](../../../04_API/API_IDENTITY.md) | `do uzupełnienia` |
| Proces | [PROC-001_AUTH.md](../../../06_PROCESY/PROC-001_AUTH.md) | `do uzupełnienia` |
| Role | [ROLE_IDENTITY.md](../../../07_ROLE_I_UPRAWNIENIA/ROLE_IDENTITY.md) | `do uzupełnienia` |
| Model danych | do uzupełnienia po analizie `03_MODEL_DANYCH` | `do uzupełnienia` |
| Testy automatyczne | do uzupełnienia po analizie `08_TESTY` | `do uzupełnienia` |
