...
**Co wyświetla / zbiera:** Etykieta kroku pipeline statusu zamówienia (np. 'Placed', 'Process', 'Dispatch', 'Transit', 'Delivered'). Pochodzi z pipelineSteps(status).
**Źródło danych:** Lokalny enum OrderStatus + stały zestaw etykiet w pipelineSteps().
**Kiedy widoczne:** W każdym wierszu tabeli, w pasku postępu statusu.
**Format:** Ciąg znaków.
