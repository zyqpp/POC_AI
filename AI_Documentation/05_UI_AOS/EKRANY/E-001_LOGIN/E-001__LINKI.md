# E-001 Linki Śladu

Status: `szkielet`; dokument służy jako centralna nawigacja dla ekranu.

## Ekran

- [Opis ekranu](E-001__README.md)
- [Pola UI](P-001_POLA/P-001__INDEX.md)
- [Akcje UI](A-001_AKCJE/A-001__INDEX.md)
- [Błędy i komunikaty](ERR-001_BLEDY/ERR-001__INDEX.md)
- [Dane testowe](TD-001_DANE_TESTOWE/TD-001__INDEX.md)
- [Testy](TC-001_TESTY/TC-001__INDEX.md)

## Powiązania Do Uzupełnienia

| Obszar | Link | Status |
|---|---|---|
| API | [API_CATALOG.md](../../../04_API/API_CATALOG.md) | `do uzupełnienia` |
| Proces | [PROC-001_AUTH.md](../../../06_PROCESY/PROC-001_AUTH.md) | `do uzupełnienia` |
| Role | do uzupełnienia po analizie `07_ROLE_I_UPRAWNIENIA` | `do uzupełnienia` |
| Model danych | [MODEL_DANYCH_CATALOG.md](../../../03_MODEL_DANYCH/MODEL_DANYCH_CATALOG.md) | `do uzupełnienia` |
| Testy automatyczne | do uzupełnienia po analizie `08_TESTY` | `do uzupełnienia` |
